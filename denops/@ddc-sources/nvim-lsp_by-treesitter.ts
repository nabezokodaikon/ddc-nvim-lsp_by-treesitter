import { Denops } from "https://deno.land/x/denops_std@v3.1.0/mod.ts";
import {
  BaseSource,
  DdcGatherItems,
  Item,
...
export class Source extends BaseSource<Params> {
  ...
  async getCompletePosition(
    ...
    let a: string[];
    try {
      a = await args.denops.call("luaeval", "require('delimities').get_delimities()") as string[];
    } catch (_) {
  ...
  async gather(
    ...
    const [payload] = await Promise.all([
      ...
      args.denops.call(
        "luaeval",
        "require('ddc_nvim_lsp_by_treesitter').request_candidates(" +
